  ...
  return `SELECT DISTINCT (STRAFTER(STR(?item), STR(wd:)) AS ?wdid)
        ...
        WHERE {
          ...
          OPTIONAL {
            ...
            ?ref pr:P4656 ?source FILTER CONTAINS(STR(?source), 'fr.wikipedia.org') .
          ...
          OPTIONAL { ?positionItem wdt:P1705  ?nativeLabel   FILTER(LANG(?nativeLabel)   = "fr") }
          OPTIONAL { ?positionItem rdfs:label ?positionLabel FILTER(LANG(?positionLabel) = "fr") }
